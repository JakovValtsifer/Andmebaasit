...
--
-- Host: 127.0.0.1
-- Loomise aeg: Aprill 17, 2024 kell 01:11 PL
...
-- Toimingud
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_filmid` (IN `pikkus_avg` INT)   BEGIN
DECLARE today timestamp DEFAULT CURRENT_DATE;
	SELECT today, filmnimetus, pikkus
    from film
    WHERE pikkus > pikkus_avg;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `get_film_by_type` (IN `type` INT(3))  DETERMINISTIC BEGIN
SELECT filmNimetus, filmtype
FROM film f
